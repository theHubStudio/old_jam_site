...
(function (global, factory) {
    ...
    */
    ...
    win.fp_utils = win.fp_utils || {}; // @ts-ignore

    Object.assign(win.fp_utils, {
      "prependTo": prependTo,
      "toggleClass": toggleClass
    });
    function isExtensionLoaded(internalName) {
      var externalName = 'fp_' + internalName + 'Extension';
      return typeof window[externalName] !== 'undefined';
    }
    function usingExtension(name) {
      var options = getOptions(); //is an array?
    ...
    function extensionCall(extensionName, method, params) {
      if (usingExtension(extensionName)) {
        if (isFunction(FP[extensionName][method])) {
          return FP[extensionName][method](params);
        }

        return FP[extensionName][method];
      }
    }
    /**
    ...
    */
    ...
    function isAnimatingDragging() {
      return extensionCall('dragAndMove', 'isAnimating');
    }
    function isDragging() {
      return extensionCall('dragAndMove', 'isGrabbing');
    }
    function getWindowHeightOffset(element) {
      return getOptions().offsetSections && FP.offsetSections ? Math.round(extensionCall('offsetSections', 'getWindowHeight', element)) : getWindowHeight();
    }
    function getDomain() {
      if (doc.domain.length) {
        var parts = doc.domain.replace(/^(www\.)/, "").split('.'); //is there a subdomain?
    ...
    function decode(value) {
      ...
      function removeCrap(value) {
        ...
        if (extensionType.length > 1) {
          ...
          return decode(domainWithoutAddedChars.slice(2).slice(0, -2)) + '_' + decode(withoutDomain.slice(3).slice(0, -3));
        }
    ...
    */
    ...
    Item.prototype["prevPanel"] = function () {
      if (this === this.prev()) {
        return this.parent ? this.parent.prev() : null;
      }
    ...
    Item.prototype["nextPanel"] = function () {
      if (this === this.next()) {
        return this.parent ? this.parent.next() : null;
      }
    ...
    */
    ...
    function setScrollingSpeed(value, type) {
      if (type !== 'internal') {
        extensionCall('fadingEffect', 'update', value);
      }

      extensionCall('cards', 'update_', value);
    ...
    */
    ...
    // import { SLIDES_CONTAINER_SEL, WRAPPER_SEL } from '../common/selectors.js';
    // EventEmitter.on('bindEvents', bindEvents);
    // function bindEvents(){
    //     var element = document.querySelector(WRAPPER_SEL);
    //     element.addEventListener("transitionend", onTransitionEnd, false);
    //     state.sections.forEach(function(section){
    //         var slideWrapper = utils.$(SLIDES_CONTAINER_SEL ,section.item)[0];
    //         if(slideWrapper){
    //             slideWrapper.addEventListener("transitionend", onTransitionEndSlide, false);
    //         }
    //     });
    // }
    ...
    //     console.warn("---------------- ENDS----------------");
    // }
    // function onTransitionEndSlide(){
    //     console.warn("---------------- ENDS SLIDE ----------------");
    // }

    /**
    * Dispatch events & callbacks making sure it does it on the right format.
    */
    ...
    */
    ...
    function landscapeScroll(slides, destiny, direction) {
      ...
      extensionCall('cards', 'apply', v);
      extensionCall('dropEffect', 'apply', v);
      extensionCall('waterEffect', 'apply', v);
      ...
      if (!usingExtension('dropEffect') || !getOptions().dropEffect) {
      ...
      extensionCall('continuousHorizontal', 'apply', v);
      ...
      if (getOptions().interlockedSlides && FP.interlockedSlides) {
        //not using continuousHorizontal or using it but not about to apply it
        if (!usingExtension('continuousHorizontal') || typeof direction === "undefined" || direction === v.xMovement) {
          extensionCall('interlockedSlides', 'apply', v);
        }
      }
    }
    /**
    ...
    */
    ...
    function performHorizontalMove(slides, v, fireCallback) {
      ...
      if (getOptions().css3) {
        ...
        FP.test.translate3dH[v.sectionIndex] = translate3d; // when using dragAndMove + interlocked slides
        // we have to make sure not to add "transition" as we are using JS for animation

        if (!usingExtension('dragAndMove') || typeof v.isInterlockedSlide === 'undefined') {
          addAnimation($(SLIDES_CONTAINER_SEL, slides));
        }

        css($(SLIDES_CONTAINER_SEL, slides), getTransforms(translate3d));
    ...
    */
    ...
    function afterSlideLoads(v) {
      extensionCall('continuousHorizontal', 'afterSlideLoads', v); //if the site is not just resizing and readjusting the slides

      if (!v.localIsResizing) {
        extensionCall('parallax', 'afterSlideLoads');
        extensionCall('scrollOverflowReset', 'setPrevious', v.prevSlide);
        extensionCall('scrollOverflowReset', 'reset');
      ...
      extensionCall('interlockedSlides', 'interlockedSlides', v);
    }
    ...
    */
    ...
    function setAutoScrolling(value, type) {
      ...
      if (getOptions().autoScrolling && !getOptions().scrollBar) {
        ...
        disableAnimation(getContainer()); //scrolling the page to the section with no animation
      ...
      trigger(getContainer(), 'setAutoScrolling', value);
    }
    ...
    */
    ...
    function createInfiniteSections(v) {
      ...
      v.yMovement = getYmovement(getState().activeSection, v.element); //sections will temporally have another position in the DOM
      //updating this values in case we need them, such as in parallax extension

      v.leavingSection = v.items.origin.index() + 1;
      v.sectionIndex = v.items.destination.index();
      trigger(getContainer(), 'onContinuousVertical', v);
      return v;
    }
    /**
    ...
    */
    ...
    function continuousVerticalFixSectionOrder(v) {
      ...
      keepSlidesPosition(); //recalculating the sections data after the changes
    ...
    */
    ...
    function scrollPage(section, callback, isMovementUp) {
      ...
      if (!v.localIsResizing) {
        ...
        if (isExtensionLoaded('dropEffect')) {
          FP.dropEffect.onLeave_(v);
        }

        if (isExtensionLoaded('waterEffect')) {
          FP.waterEffect.onLeave_(v);
        }
      ...
      extensionCall('cards', 'apply', v);
      extensionCall('dropEffect', 'apply', v);
      extensionCall('waterEffect', 'apply', v); // If continuousVertical && we need to wrap around
      ...
      extensionCall('scrollOverflowReset', 'setPrevious', getState().activeSection.item); //pausing media of the leaving section (if we are not just resizing, as destinatino will be the same one)
    ...
    */
    ...
    function getBulletIndex(v) {
      ...
      if (v.wrapAroundElements != null) {
        return v.isMovementUp ? state.numSections - 1 : 0;
    ...
    */
    ...
    function getDestinationPosition(element) {
      ...
      var isScrollingDown = usingExtension('dragAndMove') && extensionCall('dragAndMove', 'isGrabbing') ? extensionCall('dragAndMove', 'isScrollingDown') : elementTop > state.previousDestTop;
      ...
      if (usingExtension('offsetSections')) {
        position = FP.offsetSections.getSectionPosition_(isScrollingDown, position, element);
      }
      /*
    ...
    */
    ...
    function performMovement(v) {
      ...
      if (getOptions().css3 && getOptions().autoScrolling && !getOptions().scrollBar) {
        ...
        transformContainer(translate3d, true); // bugfix for scrolling fast on waterEffect: 'slides'

        if (usingExtension('waterEffect')) {
    ...
    */
    ...
    function afterSectionLoads$1(v) {
      ...
      extensionCall('waterEffect', 'afterLoad');
      extensionCall('dropEffect', 'afterLoad');
      extensionCall('scrollOverflowReset', 'reset');
      extensionCall('resetSliders', 'apply', v);
      updateState();
    ...
    function setResponsive(active) {
      ...
      if (active) {
        if (!isResponsive) {
          ...
          extensionCall('responsiveSlides', 'toSections');
          trigger(getContainer(), 'afterResponsive', active);
        }
        ...
        extensionCall('responsiveSlides', 'toSlides');
        trigger(getContainer(), 'afterResponsive', active);
      }
    }
    /**
    ...
    */
    ...
    function styleSection(section) {
      ...
      if (usingExtension('offsetSections')) {
        css(sectionElem, {
          'height': getWindowHeightOffset(sectionElem) + 'px'
        });
      }
    ...
    */
    ...
    function updateState() {
      ...
      trigger(getContainer(), 'onUpdateStateDone');
    }
    function updateStructuralState() {
      var allSectionItems = $(getOptions().sectionSelector + ', ' + SECTION_SEL, getContainer());
    ...
    function scrollToNewActivePanel() {
      ...
      if (!activeSection && state.sections.length && !getState().isBeyondFullpage) {
        if (g_prevActiveSectionIndex) {
    ...
    */
    ...
    function styleSlides(section) {
      ...
      var startingSlide = usingExtension('responsiveSlides') ? null : section.activeSlide || null; //if the slide won't be an starting point, the default will be the first one
      //the active section isn't the first one? Is not the first slide of the first section? Then we load that section/slide by default.

      if (startingSlide != null && state.activeSection && (state.activeSection.index() !== 0 || state.activeSection.index() === 0 && startingSlide.index() !== 0)) {
        silentLandscapeScroll(startingSlide.item, 'internal');
        addClass(startingSlide.item, INITIAL);
      } else {
    ...
    function didSlidesChange() {
      return extensionCall('responsiveSlides', 'isResponsiveSlidesChanging') || getVisible($(getOptions().slideSelector, getContainer())).length !== getState().numSlides;
    }

    function didSectionsChange() {
      return extensionCall('responsiveSlides', 'isResponsiveSlidesChanging') || getVisible($(getOptions().sectionSelector, getContainer())).length !== getState().numSections;
    }
    ...
    */
    ...
    function scrolling(type) {
      ...
      if (usingExtension('scrollHorizontally')) {
        scrollSection = extensionCall('scrollHorizontally', 'getScrollSection', {
          "type": type,
    ...
    */
    ...
    function addTouchHandler() {
      ...
      if (isTouchDevice || isTouch) {
        //when not using dragAndMove or only using it for mouse
        if (!usingExtension('dragAndMove') || getOptions().dragAndMove === 'mouseonly') {
          if (getOptions().autoScrolling) {
    ...
    */
    ...
    function touchMoveHandler(e) {
      ...
      if (isReallyTouch(e)) {
        ...
        var direction = isHorizontalPredominantMove ? directionH : directionV; // if(getOptions().allowCorrectDirection){
        //     setState({
        //         canScroll: state.canScroll && state.touchDirection == direction || state.touchDirection !== direction,
        //         slideMoving: state.slideMoving && state.touchDirection == direction || state.touchDirection !== direction
        //     });
        // }
    ...
    */
    ...
    function moveSlide(direction, section) {
      ...
      var slides = $(SLIDES_WRAPPER_SEL, activeSectionItem)[0]; // if(getOptions().allowCorrectDirection && usingExtension('scrollHorizontally')){
      //     setState({
      //         slideMoving: state.slideMoving && state.sliderDirection === direction,
      //         canScroll: false
      //     });
      // }
      // setState({sliderDirection: direction});
      // more than one slide needed and nothing should be sliding

      if (slides == null || isAnimatingDragging() || state.slideMoving || activeSection.slides.length < 2) {
        return;
      }
    ...
    function MouseWheelHandler(e) {
      ...
      if (getOptions().autoScrolling && !getControlPressed() && !isNormalScroll) {
        ...
        if (timeDiff > 200) {
          //emptying the array, we dont care about old scrollings for our averages
          scrollings = [];
        } // EventEmitter.emit('allowCorrection', {type: 'wheel', direction: direction});
        ...
        if (state.canScroll && !isAnimatingDragging()) {
    ...
    */
    ...
    function resizeActions() {
      ...
      trigger(getContainer(), 'onResizeEnds');
      setState({
        isResizing: false
      });
    }
    ...
    function reBuild(resizing) {
      ...
      for (var i = 0; i < sections.length; ++i) {
        ...
        if (usingExtension('offsetSections')) {
          css(section.item, {
            'height': getWindowHeightOffset(section.item) + 'px'
          });
      ...
      if (!state.isBeyondFullpage) {
        ...
        if (sectionIndex && !usingExtension('fadingEffect') && !usingExtension('dropEffect') && !usingExtension('waterEffect')) {
    ...
    */
    ...
    function setSectionsHeight(value) {
      getState().sections.forEach(function (section) {
        var propertyValue = value === '' ? '' : getWindowHeightOffset(section.item) + 'px';
        css(section.item, {
    ...
    */
    ...
    function hashChangeHandler() {
      if (!state.isScrolling && !getOptions().lockAnchors) {
        ...
        if (sectionAnchor && sectionAnchor.length) {
          ...
          if (sectionAnchor && sectionAnchor !== state.lastScrolledDestiny && !isFirstSlideMove || isFirstScrollMove && !isAnimatingDragging() || !state.slideMoving && state.lastScrolledSlide != slideAnchor && !isAnimatingDragging()) {
            EventEmitter.emit(events.onScrollPageAndSlide, {
    ...
    function scrollHandler(e) {
      ...
      if ((!getOptions().autoScrolling || getOptions().scrollBar || usingExtension('dragAndMove')) && !isDragging()) {
        var currentScroll = usingExtension('dragAndMove') ? Math.abs(extensionCall('dragAndMove', 'getCurrentScroll')) : getScrollTop();
        var scrollDirection = getScrollDirection(currentScroll);
        var visibleSectionIndex = 0;
        var screen_mid = currentScroll + getWindowHeight() / 2.0;
        var documentHeight = usingExtension('dragAndMove') ? extensionCall('dragAndMove', 'getDocumentHeight') : $body.scrollHeight - getWindowHeight();
        ...
        if (isAtBottom) {
          ...
          for (var i = 0; i < sections.length; ++i) {
            // for responsiveSlides ? 
            var section = closest(sections[i].item, SECTION_SEL) || sections[i].item; // Pick the the last section which passes the middle line of the screen.
        ...
        var isCurrentSectionActive = currentSection.isActive; //setting the visible section as active when manually scrolling
        ...
        if (!isCurrentSectionActive) {
          ...
          if (state.canScroll) {
            // for responsiveSlides?
            var sectionSiblings = sections.filter(function (section) {
              return section.index() !== currentSection.index();
            }).map(function (section) {
              return section.item;
            });
            removeClass(sectionSiblings, ACTIVE);
            ...
            extensionCall('resetSliders', 'apply', {
              localIsResizing: state.isResizing,
              leavingSection: leavingSectionIndex
            });
    ...
    */
    ...
    function prepareDom() {
      ...
      enableYoutubeAPI();
      extensionCall('fadingEffect', 'apply');
      extensionCall('waterEffect', 'init');
      extensionCall('dropEffect', 'init');
      extensionCall('cards', 'init');
    ...
    */
    ...
    function afterRenderActions() {
      ...
      trigger(getContainer(), 'afterRender');
    }
    /**
    ...
    */
    ...
    function setAllowScrolling(value, directions) {
      ...
      trigger(getContainer(), 'setAllowScrolling', {
        value: value,
    ...
    */
    ...
    function destroyStructure() {
      ...
      disableAnimation(getContainer()); //Unwrapping content
    ...
    function isValidActivationKey(extensionName) {
      var isKeyDefined = typeof getActivationKey()[extensionName] !== 'undefined' && getActivationKey()[extensionName].length;
      var arrayKey = [];
      var isValid = false; //do we have an array as key? ['key1', 'key2']... let's check them all!

      if (!isArrayOrList(getActivationKey()[extensionName])) {
        arrayKey.push(getActivationKey()[extensionName]);
      } else {
        arrayKey = getActivationKey()[extensionName];
      }

      arrayKey.forEach(function (key) {
        var domainName = getDomain();
        var _0x86bb = ['MTM0bG9jYWxob3N0MjM0', 'MTM0MC4xMjM0', 'MTM0anNoZWxsLm5ldDIzNA==', 'UDdDQU5ZNlNN', 'NTY3YnVuZGxlNzg5', 'NTU1S2V5Nzc3'];
        var localhost = decode(_0x86bb[0]);
        var localIp = decode(_0x86bb[1]);
        var jsfiddle = decode(_0x86bb[2]);
        var unlimited = decode(_0x86bb[3]);
        var bundle = decode(_0x86bb[4]); // bundle

        var keyText = decode(_0x86bb[5]); //Key

        var hasBundleKey = typeof getOptions()[bundle + keyText] !== 'undefined';
        isKeyDefined = isKeyDefined || hasBundleKey;
        var isNotTest = [localhost, localIp, jsfiddle].indexOf(domainName) < 0 && domainName.length !== 0; // to remove
        // isNotTest = true;

        if (!isKeyDefined && !hasBundleKey && isNotTest) {
          return false;
        }

        var decoded = isKeyDefined ? decode(key) : ''; // [0] = domain   [1] = extensionName
        ...
        var isBundleKey = decoded.length > 1 && decoded[1].toLowerCase().indexOf(bundle) > -1;
        var domainDoesntMatch = decoded[0].indexOf(domainName, decoded[0].length - domainName.length) < 0;
        var isExtensionValid = extensionMatches || isBundleKey;
        isValid = isValid || !(domainDoesntMatch && isNotTest && unlimited != decoded[0]) && isExtensionValid || !isNotTest;
      });
      return isValid;
    }

    var isGutenbergValid = !hasClass($body, decode('OHNsd3AtZnVsbHBhZ2UtanM5T20='));

    var bannerContent;
    var objectElement;
    var destroyedId;
    var isUnlicensesBannerAdded;
    function checkActivationKey(extensionName) {
      objectElement = doc.createElement('div');
      ...
      if (!isGutenbergValid) {
        bannerContent = bannerContent.replace('extensions/', '').replace('Extension', '');
      }
      ...
      if (isGutenbergValid && (!usingExtension(extensionName) || !FP[extensionName])) {
        return;
      }

      if (!isValidActivationKey(extensionName) || !isGutenbergValid) {
        addWarning();
        var setIntervalFunction = decode('MzQ1c2V0SW50ZXJ2YWwxMjM='); //https://jsfiddle.net/youxkay0/
        ...
        window[setIntervalFunction](addWarning, 2 * 1000);
      }
    }
    ...
    function onBannerChange(mutations) {
      mutations.forEach(function (mutation) {
        if (mutation.removedNodes[0] && mutation.removedNodes[0].isEqualNode(objectElement)) {
          ...
          var setTimeoutFunction = decode('bDIwc2V0VGltZW91dDAzbA=='); // @ts-ignore

          destroyedId = window[setTimeoutFunction](destroyed, 900);
        }
      });
    }
    ...
    */
    ...
    function destroy(all) {
      trigger(getContainer(), 'destroy', all);
      setAutoScrolling(false, 'internal');
      setAllowScrolling(true);
      setMouseHijack(false);
      ...
      extensionCall('dragAndMove', 'destroy'); //lets make a mess!
    ...
    function getFullpageData() {
      return {
        ...
        "internals": {
          ...
          "isScrollAllowed": getIsScrollAllowed(),
          ...
          "keepSlidesPosition": keepSlidesPosition$1,
          ...
          "updateStructuralState": updateStructuralState,
          ...
          "setActiveSection": function setActiveSection(activeSection) {
    ...
    * externalName {String} Extension name with the form fp_[NAME]Extension referring to an external function.
    */

    function loadExtension(internalName) {
      var _0x86bb = ['NTY3YnVuZGxlNzg5', 'NTU1S2V5Nzc3'];
      var bundle = decode(_0x86bb[0]); // bundle

      var keyText = decode(_0x86bb[1]); //Key

      var hasBundleKey = typeof getOptions()[bundle + keyText] !== 'undefined';
      var externalName = 'fp_' + internalName + 'Extension';

      if (hasBundleKey) {
        getActivationKey()[internalName] = getOptions()[bundle + keyText];
      } else {
        getActivationKey()[internalName] = getOptions()[internalName + keyText];
      }
    ...
    function fullpage(containerSelector, options) {
      ...
      if (getContainer()) {
        EventEmitter.emit(events.beforeInit); //Loading extensions
        ...
        loadExtension('cards');
        loadExtension('dropEffect');
        loadExtension('waterEffect');
        extensionCall('dragAndMove', 'init');
        extensionCall('responsiveSlides', 'init');
        init();
        EventEmitter.emit(events.bindEvents);
        extensionCall('dragAndMove', 'turnOffTouch');
      } // @ts-ignore
    ...
    function setAPI() {
      FP.getFullpageData = getFullpageData; //public functions
      ...
      win.fullpage_api = FP; //used to determine whether the extensions file is loaded or not
      // @ts-ignore

      win.fullpage_extensions = true;
    }
